...
class PAM_Module(nn.Module):
    ...
    def forward(self, x, x1, flag=False, MS=True):
        ...
        #print('input x1 shape is', x1.shape)
...
class PANet(nn.Module):
    ...
    def forward(self, x):
        ...
        pam_4, down_4 = self.PAM_4(c4, c4, True, True) # [8,2048,16,16] [8,2048,16,16] -> [8,512,16,16] [8,512,16,16]
        pam_3, down_3 = self.PAM_3(c3, down_4, False, True)#[8,1024,32,32] [8,512,16,16] -> [8,512,16,16] [8,512,16,16]
        pam_2, down_2 = self.PAM_2(c2, down_3, False, True)#[8,512,64,64] [8,512,16,16] -> [8,512,16,16] [8,512,16,16]

        x = torch.cat([pam_2, pam_3, pam_4, c4], 1)  #[8, 512*3, 16, 16] [8, 2048, 16, 16]
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.fc_(x)
